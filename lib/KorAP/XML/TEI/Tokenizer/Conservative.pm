...
sub _add_surroundings {
  ...
  if ($p2 == $p1+1) { # single punctuation character
    ...
    if ($preceding) {
      ...
      # NOTE: this fixes the bug with '.Der', where '.' was not tokenized (see t/tokenization.t)
      ...
      $pr = ( substr( $txt, $p2, 1 ) =~ /^[^A-Za-z0-9]?$/ ); # the last punctuation character should always be tokenized (signified by the ?)
