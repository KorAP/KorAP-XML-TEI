  ...
  while (<$fh>) {
    ...
    if ( m#^(.*)<$sig_type(?: [^>]*)?>([^<]*)(.*)$# ){
      ...
      die "ERROR ($0): main(): input line number $.: line with sigle-tag is not in expected format ... => Aborting\n\tline=$_"
        if $pfx !~ /^\s*$/  || $sfx !~ m#^</$sig_type>\s*$# || $dir =~ /^\s*$/;

      my $sig = encode_utf8($dir);
      $self->[SIGLE] = $sig;

      # Transliterate directory
      $dir =~ tr/\./\//;
